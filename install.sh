...
# @@File             :  install.sh
# @@Description      :  Install configurations for emacs
...
GLOBAL_OS_PACKAGES="emacs clang rg fdfind"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__run_post_install() {
  local getRunStatus=0
  SET_EMACSVER="$(emacs --version | sed 's|.*[eE]macs ||g' | grep '[0-9]' | head -n 1 || echo '0')"
  ...
  if [ "$EMACSVER" -lt "27" ]; then
    ...
    __cp_rf $INSTDIR/etc/. "$HOME/.config/doom/"
  fi
  if [ -x "$PLUGIN_DIR/doom-emacs/bin/doom" ]; then
    "$PLUGIN_DIR/doom-emacs/bin/doom" sync --force
    "$PLUGIN_DIR/doom-emacs/bin/doom" env --force
    "$PLUGIN_DIR/doom-emacs/bin/doom" install --install --no-env --force
    "$PLUGIN_DIR/doom-emacs/bin/doom" sync --force
  fi
  return $getRunStatus
}
